...
- 采样率：500 Hz
- 使用通道：前 30 个通道
- 预处理：逐 trial、逐通道去均值，50 Hz 陷波，4–40 Hz Butterworth 零相位带通滤波
- 信号归一化：zscore_per_trial_channel
- 时间窗：(1.5, 5.5)

## 特征与验证方法

- 特征：Filter-Bank CSP 空间判别特征；每个训练折内部按子频带重新拟合。
- 特征选择：mibif，保留前 24 维，并在每个训练折内部拟合。
...
样本数：240；类别：左手运动想象、右手运动想象；特征维数：24。

| 模型 | Accuracy | Balanced Accuracy | Macro F1 |
|---|---:|---:|---:|
| LDA | 0.5458 | 0.5458 | 0.5456 |

按 Balanced Accuracy，本次实验表现最佳的模型为 **LDA**（0.5458）。
